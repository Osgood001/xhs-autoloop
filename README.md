...
- **Prompt**：最直接的约束。"不要用 Markdown"、"标题不超过 15 字"——这些是每轮都要执行的硬规则，写在 prompt 里每次注入。
- **Skill**：可复用的能力模块。当你发现 Agent 反复需要某个复杂流程（比如"从 HN 抓热点 → 筛选 AI 相关 → 改写为小红书风格"），把它封装成 Claude Code 的 [Skill](https://docs.anthropic.com/en/docs/claude-code/skills)，Agent 下次直接调用，不用每次重新摸索。
- **脚本**：Agent 之外的自动化。Watchdog、数据采集、状态检查——这些不该依赖 Agent 自己实现，而是用确定性的 shell 脚本保证执行。Agent 负责"思考和创作"，脚本负责"调度和保活"。

```
培养的产出物：
  Prompt   → 每轮注入的规则和策略（config/prompt-template.md）
  Skill    → 可复用的能力封装（~/.claude/skills/）
  脚本     → 确定性的调度和保活（scripts/）
```
...
- 从数据中学到的反思框架（赞藏比分析）
- 稳定运行的调度脚本（watchdog + cron + tmux 三件套）
